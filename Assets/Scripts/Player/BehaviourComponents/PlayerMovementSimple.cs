...
public class PlayerMovementSimple : PlayerComponent
{
    float maxSpeed, acceleration, deceleration, currSpeed;
    Vector2 myDirection;
    Vector2 velocity => rigidbody.velocity;
    public PlayerMovementSimple(Player player) : base(player)
    {
        myDirection = Vector2.zero;
        ComponentAction = ChangeVelocity;
    }

    public PlayerMovementSimple(float maxSpeed, float acceleration, float deceleration, Player player) : this(player){
